...
> **Update (July 2026):** Google has begun retiring this design on two fronts. **Keys:** since **June 19, 2026** the Gemini API rejects unrestricted standard keys, and by **September 2026** standard keys give way to service-account-backed [auth keys](https://ai.google.dev/gemini-api/docs/api-key) with built-in leaked-key enforcement. **Budgets:** on **July 29, 2026** Google launched [spend caps on budgets](https://cloud.google.com/blog/topics/cost-management/new-early-anomalies-and-spend-caps-on-google-cloud-budgets) (Public Preview) — budgets that actually pause usage within minutes of the threshold — plus pre-invoice **Early Anomalies** alerts, both covering Gemini API, Agent Platform, Cloud Run, and Cloud Run functions. That validates this project's threat model twice over; it does not close it: spend caps are console-only (no gcloud/API surface), scoped to one project + one service each, and limited to those four services — every other GCP API still accepts unrestricted keys and still bills uncapped with email-only budgets — overage during the enforcement lag is still billed, and fraud predating the change still needs disputing.
...
**v1.3.0** — works end-to-end. Tested against a multi-project GCP organization. 150 tests in CI — 96 for the `gcp-finops` MCP server (pytest matrix on Python 3.11/3.12/3.13) and 54 for the `gcp-dispute-kit` skill (including a synthetic-fixture end-to-end packet run) — plus schema validation, `bandit` static analysis, and `pip-audit` CVE scan, all required to pass. Pull requests welcome.
